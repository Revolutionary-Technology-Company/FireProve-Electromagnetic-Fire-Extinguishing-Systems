#!/usr/bin/env python3
"""
FireProve Turret System - Hexadecimal Target Vector Processor
Parses TDoA matrices from the 4-microphone pyramidal array and maps output vectors
directly onto native 16-state analog logic voltage intervals (0.0V - 1.0V).
"""

import math

class HexTargetProcessor:
    ...
    def compute_spatial_angles(self, t1, t2, t3, t4):
        """
        Calculates Azimuth and Elevation via differential time arrivals.
        Mic 1: Up-Left, Mic 2: Up-Right, Mic 3: Down-Left, Mic 4: Down-Right
        """
        # Vertical differential delta vector (Elevation extraction)
        delta_t_vert = (t1 + t2) - (t3 + t4)
        # Horizontal differential delta vector (Azimuth extraction)
        delta_t_horiz = (t1 + t3) - (t2 + t4)
        
        # Geometrical angular error calculations
    ...
    def calculate_motor_voltages(self, az_error, el_error):
        """
        Maps structural angular tracking errors directly to 0-1V motor control rails.
